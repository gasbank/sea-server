...
namespace ss {
    ...
    private:
        ...
        static bool query_rtree(rtree_t* rtree, const std::vector<seaarea_entry>& entries, float lng, float lat, std::string& name);
        static int contains_point_in_polygon(const seaarea_entry& ent, float testx, float testy);
        static void init_area_data(boost::interprocess::mapped_region& region, std::vector<seaarea_entry>& entries, rtree_t* rtree);

        bi::managed_mapped_file seaarea_rtree_file;
        allocator_t seaarea_alloc;
        rtree_t* seaarea_rtree;
