...
namespace ss {
    ...
    public:
        ...
        std::vector<xy32> calculate_waypoints(const sea_static_object::point_t& from, const sea_static_object::point_t& to) const;
    ...
    private:
        std::vector<sea_static_object::value_t> query_tree_ex(int xc, int yc, int half_lng_ex, int half_lat_ex) const;
        std::vector<sea_static_object::value_t> query_tree(int xc0, int yc0, int xc1, int yc1) const;
        void mark_sea_water(sea_static_object::rtree_t* rtree);
        void update_chunk_key_ts(int xc0, int yc0);
        bi::managed_mapped_file land_file;
        sea_static_object::allocator_t land_alloc;
        sea_static_object::rtree_t* land_rtree_ptr;
        bi::managed_mapped_file water_file;
        sea_static_object::allocator_t water_alloc;
        sea_static_object::rtree_t* water_rtree_ptr;

        //bi::managed_mapped_file sea_water_set_file;
        //sea_static_object::allocator_t sea_water_set_alloc;

        const int res_width;
        const int res_height;
        const float km_per_cell;
        //typedef std::unordered_set<int, std::hash<int>, std::equal_to<int>, sea_static_object::allocator_t> sea_water_set_t;
