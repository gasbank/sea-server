...
namespace ss {
    ...
    public:
        ...
        city_object::point_t get_city_point(int id) const;
        city_object::point_t get_city_point(const char* name) const;
        ...
        void update();
    private:
        std::vector<city_object::value_t> query_tree_ex(int xc, int yc, int half_lng_ex, int half_lat_ex) const;
        void update_chunk_key_ts(int xc0, int yc0);
        bi::managed_mapped_file file;
        city_object::allocator_t alloc;
        city_object::rtree_t* rtree_ptr;
        const int res_width;
        ...
        std::unordered_map<int, city_object::point_t> id_point; // city ID -> city position
